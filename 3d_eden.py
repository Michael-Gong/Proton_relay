...
def processplot(n): 
    from_path = './wire_a50_r008_n30/'
    to_path   = './wire_a50_r008_n30_fig/'
    x_start=0; x_stop=280; y_start=0; y_stop=160; z_start=0; z_stop=160;
...
#    print('here3')
    ...
    ax.set_ylim([-(y_stop-y_start)/2/20,(y_stop-y_start)/2/20])
    ax.set_zlim([-(z_stop-z_start)/2/20,(z_stop-z_start)/2/20])
    ...
    im2=ax.contourf(ex.T, Y.T, Z.T, levels=levels, norm=mcolors.Normalize(vmin=0, vmax=eee), cmap=cm.pink_r, zdir='x', offset=x_start/20-2)
...
#    ax.set_ylim([-(z_stop-z_start)/2/12,(z_stop-z_start)/2/12])
    ...
    ex[ex>eee] = eee
    levels = np.linspace(0, eee, 40)
    ax.contourf(X.T, ex.T, Z.T, levels=levels, norm=mcolors.Normalize(vmin=0, vmax=eee), cmap=cm.pink_r, zdir='y', offset=(y_stop-y_start)/2/20)
#    ax.set_xlim([x_start/20-5,x_stop/20-5])
...
#    ax.set_ylim([-(z_stop-z_start)/2/12,(z_stop-z_start)/2/12])
    ...
    ex[ex>eee] = eee
    levels = np.linspace(0, eee, 40)
    im2=ax.contourf(X.T, Y.T, ex.T, levels=levels, norm=mcolors.Normalize(vmin=0, vmax=eee), cmap=cm.pink_r, zdir='z', offset=-(z_stop-z_start)/2/20)
#    ax.set_xlim([x_start/20-5,x_stop/20-5])
#    ax.set_ylim([-(y_stop-y_start)/2/12,(y_stop-y_start)/2/12])
