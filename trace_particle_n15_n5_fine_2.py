...
to_path   = './spin_a70_n15_n5_fine_2/'
...
part13_id = part13_id[ (px>0.5) & (grid_r<0.5) & (theta<10.0) ]
...
ex_d = np.zeros([part_id.size,stop-start+1])
ey_d = np.zeros([part_id.size,stop-start+1])
ez_d = np.zeros([part_id.size,stop-start+1])
bx_d = np.zeros([part_id.size,stop-start+1])
by_d = np.zeros([part_id.size,stop-start+1])
bz_d = np.zeros([part_id.size,stop-start+1])
...
for n in range(start,stop+step,step):
    ...
    ex = data['Particles/Part_ex/subset_high_p/ion_s'].data/exunit
    ey = data['Particles/Part_ey/subset_high_p/ion_s'].data/exunit
    ez = data['Particles/Part_ez/subset_high_p/ion_s'].data/exunit
    bx = data['Particles/Part_bx/subset_high_p/ion_s'].data/bxunit
    by = data['Particles/Part_by/subset_high_p/ion_s'].data/bxunit
    bz = data['Particles/Part_bz/subset_high_p/ion_s'].data/bxunit
    ...
    ex = ex[np.in1d(temp_id,part_id)]
    ey = ey[np.in1d(temp_id,part_id)]
    ez = ez[np.in1d(temp_id,part_id)]
    bx = bx[np.in1d(temp_id,part_id)]
    by = by[np.in1d(temp_id,part_id)]
    bz = bz[np.in1d(temp_id,part_id)]
    ...
    for ie in range(part_id.size):
        ...
        ex_d[ie,n-start] = ex[temp_id==part_id[ie]]
        ey_d[ie,n-start] = ey[temp_id==part_id[ie]]
        ez_d[ie,n-start] = ez[temp_id==part_id[ie]]
        bx_d[ie,n-start] = bx[temp_id==part_id[ie]]
        by_d[ie,n-start] = by[temp_id==part_id[ie]]
        bz_d[ie,n-start] = bz[temp_id==part_id[ie]]
...
np.savetxt(to_path+part_name[-5:]+'_ex.txt',ex_d)
np.savetxt(to_path+part_name[-5:]+'_ey.txt',ey_d)
np.savetxt(to_path+part_name[-5:]+'_ez.txt',ez_d)
np.savetxt(to_path+part_name[-5:]+'_bx.txt',bx_d)
np.savetxt(to_path+part_name[-5:]+'_by.txt',by_d)
np.savetxt(to_path+part_name[-5:]+'_bz.txt',bz_d)
